...
#C.snapshot_dir = osp.abspath(osp.join(C.log_dir, "snapshot_"+C.channels))
C.snapshot_dir = osp.abspath(osp.join(C.log_dir+"_"+C.channels, "snapshot"))
...
C.dataset_path = "~/FlameI"
C.img_root_folder = C.dataset_path + 'rgb/'
C.gt_root_folder = C.dataset_path + 'annotations_alt/'
...
C.num_classes = 2
C.background = 0
C.target_size  = base_config.image_size
C.image_height = base_config.image_height  # 768
C.image_width  = base_config.image_width # 768 * 2
...
C.eval_flip = False
C.eval_height = C.image_height
C.eval_width = C.image_width
