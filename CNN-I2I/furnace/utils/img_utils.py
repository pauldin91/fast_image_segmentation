...
def random_scale_i2i(img, gt, i2i_gt, scales):
    scale = random.choice(scales)
    sh = int(img.shape[0] * scale)
    sw = int(img.shape[1] * scale)
    img = cv2.resize(img, (sw, sh), interpolation=cv2.INTER_LINEAR)
    gt = cv2.resize(gt, (sw, sh), interpolation=cv2.INTER_NEAREST)
    i2i_gt = cv2.resize(i2i_gt, (sw, sh), interpolation=cv2.INTER_LINEAR)

    return img, gt, i2i_gt, scale
...
def random_mirror_i2i(img, gt, i2i_gt):
    if random.random() >= 0.5:
        img = cv2.flip(img, 1)
        gt = cv2.flip(gt, 1)
        i2i_gt = cv2.flip(i2i_gt,1)

    return img, gt, i2i_gt
